...
describe("/blogs", () => {
  ...
  describe("GET /", () => {
    ...
    it("Should return only the articles that match the category id filter", async () => {
      const res = await request(server).get(`/blogs?category=${articleCategory2._id}`);
    ...
    it("Should return the correct metadata when providing an offset", async () => {
      const res = await request(server).get(`/blogs?offset=1`);
    ...
    it("Should return the correct metadata that matches offset and the category id filter", async () => {
      const res = await request(server).get(`/blogs?category=${articleCategory2._id}&offset=1`);
    ...
    it("Should return the multiple of articles that match the tag search", async () => {
      const res = await request(server).get(`/blogs?search=common1`);
    ...
    it("Should return the singular article that matches the tag search", async () => {
      const res = await request(server).get(`/blogs?search=beaver`);
    ...
    it("Should return the singular article that matches the title search", async () => {
      const res = await request(server).get(`/blogs?search=dog`);
    ...
    it("Should return the multiple articles that matches the title search", async () => {
      const res = await request(server).get(`/blogs?search=jumped`);
    ...
    it("Should return the singular article that matches the description search", async () => {
      const res = await request(server).get(`/blogs?search=cowiest`);
  ...
  describe("GET /:id", () => {
    it("should return a article if valid id is passed", async () => {
      let articleCategory = new BlogCategory({ name: "Fiction", slug: "fiction" });
      ...
      await article.save();
      const res = await request(server).get("/blogs/" + article._id);
  ...
  describe("POST /", () => {
    ...
    const exec = () => {
      return request(server).post("/blogs").set("x-auth-token", token).send(article);
    };
    ...
    it("should save the article if it is valid", async () => {
      ...
      const article = await Blog.find({ title: "The dogiest of dogs" });
  ...
  describe("PUT /", () => {
    ...
    beforeEach(async () => {
      articleCategory = new BlogCategory({ name: "Fiction", slug: "fiction" });
      articleCategory = await articleCategory.save();

      existingArticle = new Blog({
    ...
    it("should save the article if it is valid", async () => {
      ...
      const article = await Blog.find({ title: "testtt1" });
    ...
    it("should update the article if input is valid", async () => {
      ...
      const updatedArticle = await Blog.findById(existingArticle._id);
    ...
    it("should return the updated article if it is valid", async () => {
      ...
      expect(res.body).toHaveProperty("contingency", article.contingency);
      expect(res.body).toHaveProperty("addressableHighlights", article.addressableHighlights);
    });
  });
  ...
  describe("DELETE /", () => {
    ...
    it("should delete the article if input is valid", async () => {
      ...
      const articleInDb = await Blog.findById(id);
